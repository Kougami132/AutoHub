...
push_config = {
    ...
    'BARK_PUSH': get_config('push.bark_push', fallback=''),                    # bark IP 或设备码，例：https://api.day.app/DxHcxxxxxRxxxxxxcm/
    'BARK_ARCHIVE': get_config('push.bark_archive', fallback=''),                 # bark 推送是否存档
    'BARK_GROUP': get_config('push.bark_group', fallback=''),                   # bark 推送分组
    'BARK_SOUND': get_config('push.bark_sound', fallback=''),                   # bark 推送声音
    'BARK_ICON': get_config('push.bark_icon', fallback=''),                    # bark 推送图标
    'BARK_LEVEL': get_config('push.bark_level', fallback=''),                   # bark 推送时效性
    'BARK_URL': get_config('push.bark_url', fallback=''),                     # bark 推送跳转URL

    'DD_BOT_SECRET': get_config('push.dd_bot_secret', fallback=''),                # 钉钉机器人的 DD_BOT_SECRET
    'DD_BOT_TOKEN': get_config('push.dd_bot_token', fallback=''),                 # 钉钉机器人的 DD_BOT_TOKEN

    'FSKEY': get_config('push.fskey', fallback=''),                        # 飞书机器人的 FSKEY

    'GOBOT_URL': get_config('push.gobot_url', fallback=''),                    # go-cqhttp
                                        # 推送到个人QQ：http://127.0.0.1/send_private_msg
                                        # 群：http://127.0.0.1/send_group_msg
    'GOBOT_QQ': get_config('push.gobot_qq', fallback=''),                     # go-cqhttp 的推送群或用户
                                        # GOBOT_URL 设置 /send_private_msg 时填入 user_id=个人QQ
                                        #               /send_group_msg   时填入 group_id=QQ群
    'GOBOT_TOKEN': get_config('push.gobot_token', fallback=''),                  # go-cqhttp 的 access_token

    'GOTIFY_URL': get_config('push.gotify_url', fallback=''),                   # gotify地址,如https://push.example.de:8080
    'GOTIFY_TOKEN': get_config('push.gotify_token', fallback=''),                 # gotify的消息应用token
    'GOTIFY_PRIORITY': get_config('push.gotify_priority', fallback=0),               # 推送消息优先级,默认为0

    'IGOT_PUSH_KEY': get_config('push.igot_push_key', fallback=''),                # iGot 聚合推送的 IGOT_PUSH_KEY

    'PUSH_KEY': get_config('push.push_key', fallback=''),                     # server 酱的 PUSH_KEY，兼容旧版与 Turbo 版

    'DEER_KEY': get_config('push.deer_key', fallback=''),                     # PushDeer 的 PUSHDEER_KEY
    'DEER_URL': get_config('push.deer_url', fallback=''),                     # PushDeer 的 PUSHDEER_URL

    'CHAT_URL': get_config('push.chat_url', fallback=''),                     # synology chat url
    'CHAT_TOKEN': get_config('push.chat_token', fallback=''),                   # synology chat token

    'PUSH_PLUS_TOKEN': get_config('push.push_plus_token', fallback=''),              # pushplus 推送的用户令牌
    'PUSH_PLUS_USER': get_config('push.push_plus_user', fallback=''),               # pushplus 推送的群组编码
    'PUSH_PLUS_TEMPLATE': get_config('push.push_plus_template', fallback='html'),       # pushplus 发送模板，支持html,txt,json,markdown,cloudMonitor,jenkins,route,pay
    'PUSH_PLUS_CHANNEL': get_config('push.push_plus_channel', fallback='wechat'),      # pushplus 发送渠道，支持wechat,webhook,cp,mail,sms
    'PUSH_PLUS_WEBHOOK': get_config('push.push_plus_webhook', fallback=''),            # pushplus webhook编码，可在pushplus公众号上扩展配置出更多渠道
    'PUSH_PLUS_CALLBACKURL': get_config('push.push_plus_callbackurl', fallback=''),        # pushplus 发送结果回调地址，会把推送最终结果通知到这个地址上
    'PUSH_PLUS_TO': get_config('push.push_plus_to', fallback=''),                 # pushplus 好友令牌，微信公众号渠道填写好友令牌，企业微信渠道填写企业微信用户id

    'WE_PLUS_BOT_TOKEN': get_config('push.we_plus_bot_token', fallback=''),            # 微加机器人的用户令牌
    'WE_PLUS_BOT_RECEIVER': get_config('push.we_plus_bot_receiver', fallback=''),         # 微加机器人的消息接收者
    'WE_PLUS_BOT_VERSION': get_config('push.we_plus_bot_version', fallback='pro'),          # 微加机器人的调用版本

    'QMSG_KEY': get_config('push.qmsg_key', fallback=''),                     # qmsg 酱的 QMSG_KEY
    'QMSG_TYPE': get_config('push.qmsg_type', fallback=''),                    # qmsg 酱的 QMSG_TYPE

    'QYWX_ORIGIN': get_config('push.qywx_origin', fallback=''),                  # 企业微信代理地址

    'QYWX_AM': get_config('push.qywx_am', fallback=''),                      # 企业微信应用

    'QYWX_KEY': get_config('push.qywx_key', fallback=''),                     # 企业微信机器人

    'TG_BOT_TOKEN': get_config('push.tg_bot_token', fallback=''),                 # tg 机器人的 TG_BOT_TOKEN，例：1407203283:AAG9rt-6RDaaX0HBLZQq0laNOh898iFYaRQ
    'TG_USER_ID': get_config('push.tg_user_id', fallback=''),                   # tg 机器人的 TG_USER_ID，例：1434078534
    'TG_API_HOST': get_config('push.tg_api_host', fallback=''),                  # tg 代理 api
    'TG_PROXY_AUTH': get_config('push.tg_proxy_auth', fallback=''),                # tg 代理认证参数
    'TG_PROXY_HOST': get_config('push.tg_proxy_host', fallback=''),                # tg 机器人的 TG_PROXY_HOST
    'TG_PROXY_PORT': get_config('push.tg_proxy_port', fallback=''),                # tg 机器人的 TG_PROXY_PORT

    'AIBOTK_KEY': get_config('push.aibotk_key', fallback=''),                   # 智能微秘书 个人中心的apikey 文档地址：http://wechat.aibotk.com/docs/about
    'AIBOTK_TYPE': get_config('push.aibotk_type', fallback=''),                  # 智能微秘书 发送目标 room 或 contact
    'AIBOTK_NAME': get_config('push.aibotk_name', fallback=''),                  # 智能微秘书  发送群名 或者好友昵称和type要对应好

    'SMTP_SERVER': get_config('push.smtp_server', fallback=''),                  # SMTP 发送邮件服务器，形如 smtp.exmail.qq.com:465
    'SMTP_SSL': get_config('push.smtp_ssl', fallback='false'),                # SMTP 发送邮件服务器是否使用 SSL，填写 true 或 false
    'SMTP_EMAIL': get_config('push.smtp_email', fallback=''),                   # SMTP 收发件邮箱，通知将会由自己发给自己
    'SMTP_PASSWORD': get_config('push.smtp_password', fallback=''),                # SMTP 登录密码，也可能为特殊口令，视具体邮件服务商说明而定
    'SMTP_NAME': get_config('push.smtp_name', fallback=''),                    # SMTP 收发件人姓名，可随意填写

    'PUSHME_KEY': get_config('push.pushme_key', fallback=''),                   # PushMe 的 PUSHME_KEY
    'PUSHME_URL': get_config('push.pushme_url', fallback=''),                   # PushMe 的 PUSHME_URL

    'CHRONOCAT_QQ': get_config('push.chronocat_qq', fallback=''),                 # qq号
    'CHRONOCAT_TOKEN': get_config('push.chronocat_token', fallback=''),              # CHRONOCAT 的token
    'CHRONOCAT_URL': get_config('push.chronocat_url', fallback=''),                # CHRONOCAT的url地址

    'WEBHOOK_URL': get_config('push.webhook_url', fallback=''),                  # 自定义通知 请求地址
    'WEBHOOK_BODY': get_config('push.webhook_body', fallback=''),                 # 自定义通知 请求体
    'WEBHOOK_HEADERS': get_config('push.webhook_headers', fallback=''),              # 自定义通知 请求头
    'WEBHOOK_METHOD': get_config('push.webhook_method', fallback=''),               # 自定义通知 请求方法
    'WEBHOOK_CONTENT_TYPE': get_config('push.webhook_content_type', fallback=''),         # 自定义通知 content-type

    'NTFY_URL': get_config('push.ntfy_url', fallback=''),                     # ntfy地址,如https://ntfy.sh
    'NTFY_TOPIC': get_config('push.ntfy_topic', fallback=''),                   # ntfy的消息应用topic
    'NTFY_PRIORITY': get_config('push.ntfy_priority', fallback='3'),                # 推送消息优先级,默认为3

    'WXPUSHER_APP_TOKEN': get_config('push.wxpusher_app_token', fallback=''),           # wxpusher 的 appToken 官方文档: https://wxpusher.zjiecode.com/docs/ 管理后台: https://wxpusher.zjiecode.com/admin/
    'WXPUSHER_TOPIC_IDS': get_config('push.wxpusher_topic_ids', fallback=''),           # wxpusher 的 主题ID，多个用英文分号;分隔 topic_ids 与 uids 至少配置一个才行
    'WXPUSHER_UIDS': get_config('push.wxpusher_uids', fallback=''),                # wxpusher 的 用户ID，多个用英文分号;分隔 topic_ids 与 uids 至少配置一个才行
}
# fmt: on
